...
class DebugGraphUXWrapper(object):
    ...
    def get_run_command(self):
        """Invoke run from ux"""
        if self._ux == FRONTEND_CURSES:
            self._cli_command = self.curses_obj.get_run_command()
            return self._cli_command.get_run_start_resp().action
        return common.UxAction.DEBUG_RUN
    ...
    def run_end(self, action, retvals):
        ...
        action = action
        if self._ux == FRONTEND_CURSES:
            return self.curses_obj.run_end(self._cli_command, retvals)
        return common.UxAction.EXIT
    ...
    def set_input(self, key, value):
        ...
        """
        if self._ux == FRONTEND_CURSES:
            self.curses_obj.set_input(key, value)
