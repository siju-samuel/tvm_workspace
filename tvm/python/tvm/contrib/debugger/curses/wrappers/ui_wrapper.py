...
from tvm.contrib.debugger import common
from tvm.contrib.debugger.curses.util import common as util
...
class LocalCLIDebugWrapperModule(framework.BaseDebugWrapperModule):
    ...
    def on_run_start(self, request):
        ...
        if self._exit_if_requested_by_user():
            return framework.OnRunStartResponse(common.UxAction.EXIT, [])
    ...
    def on_run_end(self, request):
        ...
        if request.performed_action == common.UxAction.DEBUG_RUN:
    ...
    def _print_input_handler(self, args, screen_info=None):
        ...
        for key in self._input_dict:
            key_name = util.get_graph_element_name(key)
    ...
    def _run_handler(self, args, screen_info=None):
        ...
        if parsed.no_debug:
            action = common.UxAction.NON_DEBUG_RUN
            debug_urls = []
        else:
            action = common.UxAction.DEBUG_RUN
    ...
    def _register_this_run_info(self, curses_cli):
        ...
        if self._input_dict:
            ...
            input_keys = [util.get_graph_element_name(key)
