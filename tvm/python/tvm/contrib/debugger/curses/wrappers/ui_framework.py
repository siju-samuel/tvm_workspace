...
class BaseDebugWrapperModule(object):
    ...
    def set_input(self, name, value):
        ...
        """
        key = name.replace("/", "_")
        self._input_dict.update({key: value})
    ...
    def run_end(self, cli_command, retvals):
        ...
        if run_start_resp.action == common.UxAction.DEBUG_RUN:
        ...
        elif run_start_resp.action == common.UxAction.NON_DEBUG_RUN:
    ...
    def get_run_command(self, run_metadata=None):
        ...
        if run_start_resp.action == common.UxAction.DEBUG_RUN or \
               run_start_resp.action == common.UxAction.NON_DEBUG_RUN or \
               run_start_resp.action == common.UxAction.EXIT:
            return run_command
        else:
            raise ValueError(
                "Invalid UxAction value: %s" % run_start_resp.action)
        return retvals
