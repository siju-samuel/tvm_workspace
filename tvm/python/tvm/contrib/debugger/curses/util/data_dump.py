...
def device_path_to_device_name(device_dir):
    ...
    device_name : str
        Parsed device name.
    """
    path_item = os.path.basename(device_dir)[len(METADATA_FILE_PREFIX) + len(DEVICE_TAG):]
    path_item = path_item.replace("ctx_", "ctx:").replace("_", ":", 1)
    return "/" + path_item
...
class DebugTensorDatum(object):
    ...
    def __init__(self, dump_root, debug_dump_rel_path):
        ...
        self._extended_timestamp = base.split("_")[-1].rpartition('.')[0]
