 ...
 * \param index The output index.
 * \param data_out the output data.
 * \return NDArray the output ndarray, corresponding to the index.
 */
NDArray GraphRuntime::GetOutputs(int index) {
  CHECK_LT(static_cast<size_t>(index), outputs_.size());
 ...
 * \return int the number of output tensors
 */
int GraphRuntime::GetOutputCount() {
